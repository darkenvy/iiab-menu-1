...
(function() {
  // debug
  var debug, forceFullDisplay, dynamicHtml;
  if (typeof debug == 'undefined') { debug = false; }
  if (typeof forceFullDisplay == 'undefined') { forceFullDisplay = false; } // allow override in index.html
  if (typeof dynamicHtml == 'undefined') { dynamicHtml = true; } // allow override in index.html
  ...
  if (isMobile && !forceFullDisplay) showFullDisplay = false;
  ...
  function procMenu() {
    ...
    for (var i = 0; i < menuItems.length; i++) { // where is menuItems coming from??
  ...
  function getMenuDef(menuItem) {
    ...
    var resp = $.ajax({
      ...
      .fail(function(jqXHR, textStatus, errorThrown) {
        var menuHtml =
          '<div class="content-item" style="padding:10px; color: red; font-size: 1.5em">' +
          menuItem +
          ' - file not found or improperly formatted</div>';
  ...
  function procMenuItem(module) {
    var menuHtml = '';
    var langClass = '';
    var calcZimLink = function(module) {
      // if kiwix_url is defined use it otherwise use port
      var href = zimVersions[module.zim_name] + '/';
      if (menuConfig.hasOwnProperty('kiwixUrl')) href = menuConfig.kiwixUrl + href;
      else href = host + ':' + menuConfig.kiwixPort + '/' + href;
    
      var html = calcLink(href, module);
      return html;
    }
    var calcHtmlLink = function(module) {
      var href = htmlBaseUrl + module.moddir;
      var html = calcLink(href, module);
      return html;
    }
    var calcWebrootLink = function(module) {
      var href = webrootBaseUrl + module.moddir;
      var html = calcLink(href, module);
      return html;
    }
    var calcKaliteLink = function(module) {
      var portRef = module.lang + '-kalitePort';
      var href = host + ':';
      if (menuConfig.hasOwnProperty(portRef)) href += menuConfig[portRef];
      else href += menuConfig['en-kalitePort'];

      var html = calcLink(href, module);
      return html;
    }
    var calcCalibreLink = function(module) {
      var href = host + ':' + menuConfig.calibrePort;
      var html = calcLink(href, module);
      return html;
    }
    var calcOsmLink = function(module) {
      var href = '/iiab/static/map.html';
      var html = calcLink(href, module);
      return html;
    }
    var calcInfoLink = function(module) {
      var href = null;
      var html = calcLink(href, module);
      return html;
    }
    var getExtraHtml = function(module) {
      if (module.hasOwnProperty('extra_html') && module['extra_html'] != '') {
        ...
        var resp = $.ajax({
          ...
          .done(function(data) {
            ...
            var menuItemHtmlfDivId = '#' + module.menu_id + '-htmlf';
    ...
    consoleLog(module);
    if (module['intended_use'] == 'zim') { menuHtml += calcZimLink(module); }
    else if (module['intended_use'] == 'html') { menuHtml += calcHtmlLink(module); }
    else if (module['intended_use'] == 'webroot') { menuHtml += calcWebrootLink(module); }
    else if (module['intended_use'] == 'kalite') { menuHtml += calcKaliteLink(module); }
    else if (module['intended_use'] == 'calibre') { menuHtml += calcCalibreLink(module); }
    else if (module['intended_use'] == 'osm') { menuHtml += calcOsmLink(module); }
    else if (module['intended_use'] == 'info') { menuHtml += calcInfoLink(module); }
    else {
      menuHtml +=
        '<div class="content-item" style="padding:10px; color: red; font-size: 1.5em">' +
        module['menu_item_name'] +
        ' - unknown module type</div>';
    }
  ...
  function calcLink(href, module) {
    ...
    if (module.hasOwnProperty('start_url')) startPage = href + '/' + module['start_url'];
    ...
    if (href != null) { html += '<a href="' + startPage + '">' + module.title + '</a>'; }
    ...
    if (module.hasOwnProperty('apk_file')) {
      var sizeClause = '';
      if (module.hasOwnProperty('apk_file_size')) { sizeClause = ' (' + module.apk_file_size + ')'; }
      if (menuConfig['apkLinkPhrase'].hasOwnProperty(module.lang)) {
        html +=
          '<p>' +
          menuConfig['apkLinkPhrase'][module.lang] +
          ' <span dir="ltr"><a href="' +
          apkBaseUrl +
          module.apk_file +
          '">' +
  ...
  function resizeHandler() {
    if (isMobile) {
      if (screen.height > screen.width) { $(':root').css('font-size', mobilePortraitSize); }
      else { $(':root').css('font-size', mobileLscapeSize); }
    }
  }
  ...
  function consoleLog(msg) {
    if (debug == true) { console.log(msg); } // for IE there can be no console messages unless in tools mode
  }
  ...
  if (dynamicHtml) {
    ...
    for (i = 0; i < menuItems.length; i++) {
      ...
      var menuItemDivId = i.toString() + '-' + menu_item_name;
      menuDefs[menu_item_name]['menu_id'] = menuItemDivId;

      html +=
        '<div id="' +
        menuItemDivId +
        '" class="content-item" dir="auto">&emsp;Attempting to load ' +
        menu_item_name +
        ' </div>';
    }
  ...
  $('#btn-submitFeedback').click(function() {
    ...
    if (validateFeedback()) sendFeedback();
  });
